...
class Database:
    ...
    def get_aps_state(self, aps_id: int) -> str:
        try:
            result = self.client.table('aps_description').select('goal_state').eq('id', aps_id).execute()
            if result.data:
                state = result.data[0]['goal_state']
                return state
        ...
        try:
            ...
            items_to_insert = [
                APSOrderItemCreate(
                    ...
                    aps_id=aps_id,
                    item_id=item_id,
                    status=ItemStatus.RESERVED_BY_CLIENT
    ...
    def remove_item_from_order(self, order_id: int, item_id: int, quantity: int) -> bool:
        try:
            ...
            items_to_remove = self.client.table('aps_order_item')\
                ...
                .eq('item_id', item_id)\
                .eq('status', ItemStatus.RESERVED_BY_CLIENT.value)\
                .limit(quantity)\
                .execute()
